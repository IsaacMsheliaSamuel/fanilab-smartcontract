...
- **`FA-6`** (`settlement_contract` scope): resolved as **temporary de-scope with a loud guard**. `get_driver_preference` returns `None` (the swap branch stays unreachable on the live payout path) and `execute_settlement_swap` unconditionally raises the typed `SettlementError::SwapNotImplemented` error, so the stub cannot silently no-op if something were ever mis-wired to call it directly. `docs/API.md`/`PRODUCTION_READINESS.md` and the crate's own doc comments are honest about this being pending Phase 3 work.
